...
| --- | --- | --- | --- |
| `SakhiDesignSystem.swift`, `PhaseColors.swift`, `PhaseColorManager.swift` | Color tokens come from KMM `SakhiColors` / `SakhiUIColors` / `ResolvedBundle`. `DesignTokens` still exposes the seven brand colour names for the iOS app, but they delegate to `SakhiColors.groupF` and no longer hold hexes of their own. On Android, read `SakhiUIColors.BRAND_*`. Brand colors in plan: `#F61887`, `#BB2968`, `#6D1743`, `#F8F2F4`, `#F8E5EC`, `#1C1C1E`, `#6B6B7A`, `#E5E4EA`, `#34C759`. | `:core:designsystem` | Build `SakhiColorScheme` from KMM first. Do not re-enter hex values in feature modules. Keep phase bundles as structured objects, not loose colors. |
